...
class ActiveCollab {
	 ...
	 * @return string
	 */
	public function setRequestString($path_info,$additional_params = null) {
	...
	protected function listCategories($project_id,$object_name) {
		if(self::getAPIVersion() > 2.0) {
			...
			$response = self::callAPI();
			if($response) {
				return self::convertXMLToArray($response,'category','ActiveCollabCategory');
			} // if
		} else {
	...
	public function listFilesByProjectId($project_id) {
		...
		$response = self::callAPI();
		if($response) {
			return self::convertXMLToArray($response,'file');
		} // if
	...
	public function listPagesByCategoryId($project_id,$category_id) {
		if(self::getAPIVersion() > 2.0) {
			...
			$response = self::callAPI();
			if($response) {
				return self::convertXMLToArray($response,'page','ActiveCollabPage');
			} // if
		} else {
			throw new ActiveCollabCommandNotRecognized(COMMAND_NOT_RECOGNIZED . 'Your API version is ' . self::getAPIVersion());
		} // if
